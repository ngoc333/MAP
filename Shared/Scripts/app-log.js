window.mapLog = (() => {
  ...
  if (typeof indexedDB === "undefined") {
    ...
    return { write: noop, get: () => Promise.resolve([]), days: () => Promise.resolve([]), clear: noop };
  }
  ...
  async function days() {
    ...
    try {
      const transaction = db.transaction(storeName, "readonly");
      const request = transaction.objectStore(storeName).index("day").getAllKeys();
  ...
  async function clear(day) {
    ...
    try {
      const transaction = db.transaction(storeName, "readwrite");
      const store = transaction.objectStore(storeName);
      store.index("day").openCursor(IDBKeyRange.only(day)).onsuccess = event => {
        const cursor = event.target.result;
        if (cursor) { cursor.delete(); cursor.continue(); }
      };
      await complete(transaction);
